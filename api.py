...
def get_random():
    cafe_objects = db.session.execute(db.select(Cafe)).scalars().all()
...
def get_all():
    cafe_objects = db.session.execute(db.select(Cafe)).scalars().all()
    return jsonify([cafe.to_dict() for cafe in cafe_objects])

@api_bp.route('/search', methods=['GET'])
def search_by_loc():
    location = request.args.get('loc')
    cafes_found = db.session.execute(db.select(Cafe).where(Cafe.location == location)).scalars().all()
    if cafes_found:
        return jsonify([cafe.to_dict() for cafe in cafes_found])
    else:
